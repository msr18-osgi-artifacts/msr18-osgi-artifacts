module org::crossminer::analysis::smells::exportNeededPackages::Evaluator
...
public int numberImportedExpPkgs(CrossminerModel model) {
...
public int numberUniqueVersionedImportedExpPkgs(CrossminerModel model) {
...
public int numberVersionedImportedExpPkgs(CrossminerModel model) 
	= (0 | it + 1 | <b, p, m> <- model.exportedPackages, size(getPackageImporters(p,m["version"],model)) > 0);
	
public int numberVersionedImportedRequiredExpPkgs(CrossminerModel model) 
...
public rel[int,int,rel[loc,set[loc]]] csvImportersExpPkgs(CrossminerModel model, bool csv=true, loc pathCSV=RESULTS_PATH) {
	invImportedPackages = invert(model.importedPackages);
...
public rel[int,int,rel[loc,str,set[loc]]] csvVersionedImportersExpPkgs(CrossminerModel model, bool csv=true, loc pathCSV=RESULTS_PATH) {
	versExportedPackages = {<p, m["version"]> | <b, p, m> <- model.exportedPackages};
